...
function selleckfileRequireFrom(wd) {
    return function selleckfileRequire(name) {
        var module, resolvedName;

        resolvedName = path.resolve(wd, name);

        try {

            // Try local file require.
            module = require(resolvedName);
            ...
            try {
                ...
                // Likely in node_modules.
                resolvedName = path.resolve(findup('node_modules', {
                    cwd: wd
                }), name);

                if (!resolvedName) { throw new Error('Unable to locate ' + name); }
...
/**
 * Find the first file named 'Selleckfile.js' in the given directory or the
 * nearest ancestor directory and invoke its exported function with our Handlebars
 * implementation to allow for custom helpers to be registered.
 *
 * @method useSelleckfileFromDirectory
 * @param {String} dir Starting directory for search.
 */
function useSelleckfileFromDirectory(dir) {
    var selleckfile,
        filePath = findup('Selleckfile.js', {
            nocase: true,
            cwd: path.resolve(dir)
        });

    if (fs.existsSync(filePath)) {
        selleckfile = require(filePath);

        selleckfile({
            require: selleckfileRequireFrom(path.dirname(filePath)),
            handlebars: Handlebars
        });
    }
}

exports.useSelleckfileFromDirectory = useSelleckfileFromDirectory;
